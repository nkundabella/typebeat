  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="p-5 rounded-3xl bg-dark-card/50 border border-white/10 backdrop-blur-md relative overflow-hidden group">
        ...
        {/* Streak & Activity */}
        <div className="p-5 rounded-3xl bg-dark-card/50 border border-white/10 backdrop-blur-md flex flex-col gap-4 mt-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['Lofi', 'Synthwave', 'EDM', 'Hip-Hop'].map((genre) => (
                <span
                  key={genre}
                  className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-medium hover:bg-neon-cyan/10 hover:border-neon-cyan/30 transition-colors cursor-pointer"
                >
                  {genre}
                </span>
              ))}
        ...
        <nav className="mt-auto flex flex-col gap-2">
          {['Dashboard', 'Library', 'Challenges', 'Shop', 'Settings', 'Profile'].map((item) => (
            <button
              key={item}
              onClick={() => {
                if (onNavigate) {
                  if (item === 'Dashboard') onNavigate(GameState.MENU);
                  else if (item === 'Profile') onNavigate(GameState.PROFILE);
                  // add other navigation handling as needed
                }
              }}
            ...
            >
              <div
                className={`w-2 h-2 rounded-full ${
                  item === 'Dashboard' ? 'bg-neon-cyan shadow-glow-cyan' : 'bg-transparent'
                }`}
              />
